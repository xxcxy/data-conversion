...
public class CustomLinkedBlockingQueue<E> extends AbstractQueue<E> implements
	...
	private void insert(Node<E> s, Node<E> e) {
		last.next = s;
		last = e;
	}
	...
	private int extract(E[] es) {
		int readsize = Math.min(es.length, count.get());
	...
	public boolean offer(E[] es, long timeout, TimeUnit unit)
			throws InterruptedException {
		...
		final ReentrantLock putLock = this.putLock;
		final AtomicInteger count = this.count;
		final int eslen = es.length;
		putLock.lockInterruptibly();
		try {
			for (;;) {
				if (count.get() + eslen <= capacity) {
					Node<E> ts = null;
					Node<E> te = null;
					for (int i = 0; i < eslen; i++) {
						Node<E> ni = new Node<E>(es[i]);
						if (i == 0)
							ts = ni;
						if (te != null)
							te.next = ni;
	...
	public E poll(long timeout, TimeUnit unit) throws InterruptedException {
		return poll(timeout,unit,null);
	}
	
	public E poll(long timeout, TimeUnit unit,E e) throws InterruptedException {
		E x = null;
		int c = -1;
		long nanos = unit.toNanos(timeout);
		final AtomicInteger count = this.count;
	...
	public int poll(E[] es, long timeout, TimeUnit unit) throws InterruptedException {
		int size = 0;
		int c = -1;
		long nanos = unit.toNanos(timeout);
		final AtomicInteger count = this.count;
	...
	private class Itr implements Iterator<E> {
		...
		Itr() {
			final ReentrantLock putLock = CustomLinkedBlockingQueue.this.putLock;
			final ReentrantLock takeLock = CustomLinkedBlockingQueue.this.takeLock;
			putLock.lock();
			takeLock.lock();
			try {
				current = head.next;
